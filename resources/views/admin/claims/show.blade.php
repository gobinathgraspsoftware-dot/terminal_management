...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        ...
        {{-- Ticket Details (BUG FIX: withTrashed() loaded in controller so this now displays correctly) --}}
        ...
        <div class="card mb-4">
            <div class="card-header">
                ...
                @if($claim->ticket->trashed())
                    <span class="badge bg-warning text-dark ms-2"><i class="bi bi-exclamation-triangle me-1"></i>Ticket Deleted</span>
                @endif
            </div>
            <div class="card-body">
                <div class="row g-3">
                    ...
                    {{-- BUG FIX: vendor_name is NOT NULL; company_name is nullable → use vendor_name as primary --}}
                    <div class="col-sm-4">
                    ...
                    <div class="col-sm-4"><div class="text-muted small mb-1">Supervisor</div><div>{{ $claim->ticket->supervisor->name ?? '-' }}</div></div>
                    ...
                    <div class="col-12"><div class="text-muted small mb-1">Merchant Address</div><div>{{ $claim->ticket->merchant_address }}</div></div>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <div class="text-muted small mt-2"><i class="bi bi-info-circle me-1"></i>Original amount: RM {{ number_format($claim->original_amount, 2) }} (adjusted by admin)</div>
                @endif
                @if($claim->admin_remarks)
                <div class="alert alert-info mt-3 mb-0 py-2"><strong>Admin Remarks:</strong> {{ $claim->admin_remarks }}</div>
                @endif
            </div>
        </div>
                    ...
                    <div class="col-sm-6">
                        <div class="d-flex align-items-center border rounded p-2">
                            ...
                            <div class="flex-grow-1 overflow-hidden">
                                ...
                                <div class="text-muted" style="font-size:.75rem">{{ $att->uploadedBy->name ?? '-' }} &middot; {{ $att->created_at ? $att->created_at->format('d/m/Y') : '' }}</div>
                            </div>
    ...
    <div class="col-lg-4">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="mb-3"><div class="text-muted small mb-1">Claimant (Technician)</div><div class="fw-semibold">{{ $claim->technician->name ?? '-' }}</div></div>
                <div class="mb-3">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <button class="btn btn-outline-secondary" id="btnUpdateAmount"><i class="bi bi-pencil"></i></button>
                    </div>
                </div>
                <div class="mb-3">
                ...
                <div class="d-grid gap-2">
                    <button class="btn btn-success btn-sm" id="btnVerify"><i class="bi bi-check-circle me-1"></i>Verify Claim</button>
                    <button class="btn btn-outline-danger btn-sm" id="btnNonClaimable"><i class="bi bi-x-circle me-1"></i>Mark Non-Claimable</button>
                </div>
            </div>
        </div>
        @endif
...
$('#btnUpdateAmount').on('click', function() {
    ...
    confirmAction('Update Amount?', 'Update claim to RM ' + parseFloat(amount).toFixed(2) + '?', function() {
        $.post('{{ route("admin.claims.update-amount", $claim->id) }}', {
            _token: '{{ csrf_token() }}', total_amount: amount, admin_remarks: $('#adminRemarks').val()
        }).done(function(r){ r.success ? (showToast(r.message,'success'), setTimeout(()=>location.reload(),1500)) : showToast(r.message,'error'); });
    });
});
$('#btnVerify').on('click', function() {
    confirmAction('Verify Claim?', 'Mark this claim as verified?', function() {
        $.post('{{ route("admin.claims.verify", $claim->id) }}', {
            _token: '{{ csrf_token() }}', total_amount: $('#adminAmount').val(), admin_remarks: $('#adminRemarks').val()
        }).done(function(r){ r.success ? (showToast(r.message,'success'), setTimeout(()=>location.reload(),1500)) : showToast(r.message,'error'); });
    });
});
$('#btnNonClaimable').on('click', function() {
    var remarks = $('#adminRemarks').val();
    if (!remarks) { showToast('Admin remarks required for non-claimable.', 'warning'); $('#adminRemarks').focus(); return; }
    confirmAction('Mark Non-Claimable?', 'This cannot be undone.', function() {
        $.post('{{ route("admin.claims.non-claimable", $claim->id) }}', {
            _token: '{{ csrf_token() }}', admin_remarks: remarks
        }).done(function(r){ r.success ? (showToast(r.message,'success'), setTimeout(()=>location.reload(),1500)) : showToast(r.message,'error'); });
    });
});
@endif

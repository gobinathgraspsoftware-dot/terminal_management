                    ...
                    <form id="ticket-claim-form">
                        ...
                        <div id="ticket-details" class="d-none">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('admin.claims.ticket-claims') }}" class="btn btn-secondary">Cancel</a>
...
$(function() {
    ...
    $('#ticket-select').on('change', function() {
        ...
        $.ajax({
            url: '{{ url("admin/claims/ajax/ticket-details") }}/' + ticketId,
            method: 'GET',
    ...
    $('#ticket-claim-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("admin.claims.store-ticket-claim") }}',
            method: 'POST',
            data: $(this).serialize(),
            success: function(res) {
                if (res.success) {
                    showToast(res.message, 'success');
                    setTimeout(function() { window.location.href = '{{ route("admin.claims.ticket-claims") }}'; }, 1500);
                } else {

...
@section('title', 'Edit Terminal Model')
...
<div class="container-fluid">
        ...
        <div class="col-md-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Edit: {{ $terminalModel->model_name }}</li>
                </ol>
            </nav>
            <h1 class="h3 mb-0">Edit Terminal Model</h1>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('admin.terminal-models.update', $terminalModel) }}" method="POST" enctype="multipart/form-data" id="editModelForm">
                @csrf
                @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control @error('model_code') is-invalid @enderror" 
                                       id="model_code" name="model_code" value="{{ old('model_code', $terminalModel->model_code) }}" readonly>
                                <div class="form-text">Model code cannot be changed</div>
                                @error('model_code')
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" class="form-control @error('model_name') is-invalid @enderror" 
                                       id="model_name" name="model_name" value="{{ old('model_name', $terminalModel->model_name) }}" required>
                                @error('model_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('brand') is-invalid @enderror" 
                                       id="brand" name="brand" value="{{ old('brand', $terminalModel->brand) }}">
                                @error('brand')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" class="form-control @error('warranty_months') is-invalid @enderror" 
                                       id="warranty_months" name="warranty_months" value="{{ old('warranty_months', $terminalModel->warranty_months) }}" 
                                       min="0" max="120">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_serial_tracked" 
                                           name="is_serial_tracked" value="1" {{ old('is_serial_tracked', $terminalModel->is_serial_tracked) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" 
                                          id="description" name="description" rows="3">{{ old('description', $terminalModel->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <select class="form-select" id="default_accessories" name="default_accessories[]" multiple>
                            @foreach($accessoryModels as $accessory)
                                <option value="{{ $accessory->id }}" 
                                    {{ in_array($accessory->id, old('default_accessories', $terminalModel->default_accessories ?? [])) ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                                       id="sort_order" name="sort_order" value="{{ old('sort_order', $terminalModel->sort_order) }}" min="0">
                                @error('sort_order')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="card mt-3">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('admin.terminal-models.show', $terminalModel) }}" class="btn btn-secondary">
                                <i class="bi bi-x-circle"></i> Cancel
                            </a>
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-save"></i> Update Terminal Model
                            </button>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    const existingSpecs = @json(old('specifications', $terminalModel->specifications ?? []));
        ...
        if (confirm('Are you sure you want to delete this image?')) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#imagePreview').html('<span class="text-muted">No image uploaded</span>');

            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <a href="{{ route('admin.quotations.print', $quotation) }}" class="btn btn-secondary btn-sm" target="_blank">
                            <i class="bi bi-printer"></i> Print
                        </a>

                        <a href="{{ route('admin.quotations.index') }}" class="btn btn-light btn-sm">
                        ...
                        <div class="col-md-6">
                            <table class="table table-sm">
                                ...
                                <tr>
                                    ...
                                    <td><a href="{{ route('admin.purchase-orders.show', $quotation->purchaseOrder) }}">{{ $quotation->purchaseOrder->po_no }}</a></td>
                                </tr>
                                @endif
                            </table>
                        </div>
...
$(document).ready(function() {
    ...
    $('#submit-approval-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.submit-approval", $quotation) }}',
                    type: 'POST',
    ...
    $('#approve-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.process-approval", $quotation) }}',
                    type: 'POST',
                    data: { 
                        _token: '{{ csrf_token() }}',
                        action: 'approve'
    ...
    $('#send-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.send", $quotation) }}',
                    type: 'POST',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {
    ...
    $('#accept-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.accept", $quotation) }}',
                    type: 'POST',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {
    ...
    $('#convert-po-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.convert-to-po", $quotation) }}',
                    type: 'POST',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {
    ...
    $('#duplicate-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.duplicate", $quotation) }}',
                    type: 'POST',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {
    ...
    $('#delete-btn').click(function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.quotations.destroy", $quotation) }}',
                    type: 'DELETE',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {

...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            ...
            <a href="{{ route('admin.stock-transfers.print', $stockTransfer) }}" 
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    @if($stockTransfer->status === 'draft')
                        ...
                        <a href="{{ route('admin.stock-transfers.edit', $stockTransfer) }}" class="btn btn-warning w-100 mb-2">
                    ...
                    @if($stockTransfer->status === 'pending_approval')
                        ...
                        <a href="{{ route('admin.stock-transfers.approve-form', $stockTransfer) }}" class="btn btn-success w-100 mb-2">
                    ...
                    @if(in_array($stockTransfer->status, ['in_transit', 'approved']))
                        ...
                        <a href="{{ route('admin.stock-transfers.receive-form', $stockTransfer) }}" class="btn btn-success w-100 mb-2">
...
function submitForApproval() {
    if (confirm('Submit this transfer for approval?')) {
        $.post('{{ route("admin.stock-transfers.submit", $stockTransfer) }}', {
            _token: '{{ csrf_token() }}'
        }).done(function() {
            location.reload();
        });
...
function cancelTransfer() {
    ...
    if (reason) {
        $.post('{{ route("admin.stock-transfers.cancel", $stockTransfer) }}', {
            _token: '{{ csrf_token() }}',
...
function rejectTransfer() {
    ...
    if (reason) {
        $.post('{{ route("admin.stock-transfers.reject", $stockTransfer) }}', {
            _token: '{{ csrf_token() }}',
...
function dispatchTransfer() {
    if (confirm('Dispatch this transfer?')) {
        ...
        $.post('{{ route("admin.stock-transfers.dispatch", $stockTransfer) }}', {
            _token: '{{ csrf_token() }}',
            lines: lines
        }).done(function() {
            location.reload();

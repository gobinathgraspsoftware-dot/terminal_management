    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="btn-group">
            @if($stockIssue->status === 'draft' && auth()->user()->can('update', $stockIssue))
                <a href="{{ route('admin.stock-issues.edit', $stockIssue->id) }}" class="btn btn-warning">
            ...
            @endif
            <a href="{{ route('admin.stock-issues.print', $stockIssue->id) }}" target="_blank" class="btn btn-secondary">
                <i class="bi bi-printer"></i> Print
            </a>
            <a href="{{ route('admin.stock-issues.index') }}" class="btn btn-outline-secondary">
...
function postIssue() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("admin.stock-issues.post", $stockIssue->id) }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
...
function cancelIssue() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("admin.stock-issues.cancel", $stockIssue->id) }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },

...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5>Job History</h5>
    <span class="badge bg-primary">{{ $site->jobOrders->count() }} Jobs</span>
</div>

@forelse($site->jobOrders as $job)
<div class="card mb-3">
    <div class="card-body">
        <div class="row align-items-center">
            <div class="col-md-8">
                <h6 class="mb-1">
                    <a href="{{ route('admin.jobs.show', $job) }}">{{ $job->job_no }}</a>
                ...
                <div class="mt-2">
                    <small class="text-muted">
                        ...
                        @foreach($job->jobAssignments as $assignment)
                            {{ $assignment->technician->name ?? 'Unknown' }}@if(!$loop->last), @endif
                        @endforeach
                    </small>
                </div>
                @endif
            ...
            <div class="col-md-4 text-end">
                <a href="{{ route('admin.jobs.show', $job) }}" class="btn btn-sm btn-outline-primary d-block">
                    <i class="bi bi-eye me-1"></i> View Job Details
                </a>

                @if($job->status === 'pending')
                <a href="{{ route('admin.jobs.assign', $job) }}" class="btn btn-sm btn-outline-warning d-block mt-1">
...
<div class="alert alert-info">
    <i class="bi bi-info-circle me-2"></i>No job history found for this site.
</div>
@endforelse

@if($site->jobOrders->count() > 10)
<div class="text-center">
    <a href="{{ route('admin.jobs.index', ['site_id' => $site->id]) }}" class="btn btn-outline-primary">
        <i class="bi bi-list me-1"></i> View All Jobs for This Site
    </a>
</div>
@endif

@extends('layouts.app')
@section('title', 'My Claim Report — TMS')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1"><i class="bi bi-receipt me-2"></i>My Claim Report</h4>
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-2"><div class="card border-0 shadow-sm"><div class="card-body py-3 text-center">
            <div class="fs-4 fw-bold text-primary">{{ number_format($stats['total'] ?? 0) }}</div><small class="text-muted">Total</small>
        </div></div></div>
        <div class="col-6 col-md-2"><div class="card border-0 shadow-sm"><div class="card-body py-3 text-center">
            <div class="fs-4 fw-bold text-success">RM {{ number_format($stats['total_amount'] ?? 0, 2) }}</div><small class="text-muted">Amount</small>
        </div></div></div>
        <div class="col-6 col-md-2"><div class="card border-0 shadow-sm"><div class="card-body py-3 text-center">
    ...
    @include('components.reports.filter-panel', [
        'filters' => ['date_range','claim_type','claim_status','claim_category'],
            ...
            <div class="table-responsive">
                <table id="reportTable" class="table table-sm table-hover mb-0" style="width:100%">
                    <thead class="table-light">
                        <tr>
                            <th>#</th><th>Claim No</th><th>Category</th><th>Ticket No</th><th>Vendor</th>
                            <th>Date</th><th class="text-end">Mileage</th><th class="text-end">Allowance</th>
...
$(document).ready(function() {
    reportTable = $('#reportTable').DataTable({
        ...
        ajax: { url: '{{ route("technician.reports.claim.data") }}', type: 'GET', data: function(d) { return $.extend(d, getReportFilters()); } },
        columns: [
            { data: null, orderable: false, render: function(d,t,r,m) { return m.row + m.settings._iDisplayStart + 1; } },
            { data: 'claim_no' }, { data: 'claim_category' }, { data: 'ticket_no' }, { data: 'vendor' },
            { data: 'claim_date' }, { data: 'total_mileage', className: 'text-end' },
            { data: 'total_allowance', className: 'text-end' }, { data: 'total_amount', className: 'text-end' },
            { data: 'status' },
        ],
        order: [], pageLength: 25, language: { emptyTable: 'No claims found.' }
    });

    $('#btnExportExcel').on('click', function() { window.location.href = '{{ route("technician.reports.claim.export") }}?' + $.param(getReportFilters()) + '&format=xlsx'; });
    $('#btnExportPdf').on('click', function() { window.location.href = '{{ route("technician.reports.claim.export") }}?' + $.param(getReportFilters()) + '&format=pdf'; });
});
</script>
@endpush

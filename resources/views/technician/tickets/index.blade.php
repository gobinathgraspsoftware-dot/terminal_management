...
$(function() {
    let table = $('#ticketsTable').DataTable({
        ...
        columns: [
            { data: 'ticket_no', render: (data, type, row) => `<a href="/technician/tickets/${row.id}" class="fw-bold text-decoration-none">${data}</a>` },
            { data: 'vendor_name' },
            ...
            { data: 'status_badge' },
            { data: 'priority_badge' },
            { data: 'sla_remaining', render: (data, type, row) => {
                if (!data) return '-';
                return `<span class="${row.sla_breached ? 'text-danger fw-bold' : 'text-success'}">${data}</span>`;
            }},
            { data: 'created_at' },
            { data: null, orderable: false, render: (data, type, row) => {
                let btns = `<a href="/technician/tickets/${row.id}" class="btn btn-sm btn-outline-primary" title="View"><i class="bi bi-eye"></i></a>`;
                if (row.status === 'assigned') {
                    btns += ` <span class="badge bg-info ms-1">Action Required</span>`;
                }
                return btns;
            }}
        ],
    ...
    $('#filterDateFrom').on('change', () => table.ajax.reload());

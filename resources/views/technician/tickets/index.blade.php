        ...
        <div id="slaAlerts" class="collapse show">
            <div class="card-body p-0" style="max-height: 200px; overflow-y: auto;">
                ...
                <div class="sla-item d-flex justify-content-between align-items-center">
                    <div>
                        <a href="{{ route('technician.tickets.show', $bt->id) }}" class="fw-bold text-danger text-decoration-none">
                            {{ $bt->ticket_no }}
                        </a>
...
$(document).ready(function() {
        ...
        columns: [
            {
                ...
                render: function(data, type, row) {
                    ...
                    return '<a href="{{ route("technician.tickets.index") }}/' + row.id + '" class="fw-bold text-decoration-none">' + data + '</a>' + sla;
                }
            },
            { data: 'vendor_name' },
            {
            ...
            { data: 'created_at' },
            {
                ...
                render: function(data, type, row) {
                    return '<a href="{{ route("technician.tickets.index") }}/' + row.id + '" class="btn btn-sm btn-outline-primary" title="View"><i class="bi bi-eye"></i></a>';
                }
            }
        ],
        order: [[6, 'desc']],
    ...
    // Filter changes
    $('#filterStatus, #filterPriority, #filterSla, #filterDateFrom, #filterDateTo').on('change', function() {
        table.ajax.reload();
    });

...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1"><i class="bi bi-sim text-secondary me-2"></i>My Accessories Usage</h4>
            <p class="text-muted mb-0">Track accessories you've used on tickets</p>
        </div>
        <div class="d-flex gap-2">
            @can('create_accessory_usage')
            <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#recordUsageModal">
                <i class="bi bi-plus-circle me-1"></i> Record Usage
            </button>
            @endcan
            <a href="{{ route('technician.inventory-management.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i> Back</a>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-sm table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            <th>#</th><th>Ticket</th><th>Type</th><th>Model</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('technician.tickets.show', $usage->ticket_id) }}" class="text-decoration-none">
                            ...
                            <td>
                                ...
                                <button type="button" class="btn btn-sm btn-outline-success return-btn" data-id="{{ $usage->id }}">
                        ...
                        @empty
                        <tr><td colspan="8" class="text-center text-muted py-4">No accessory usage records.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<div class="modal fade" id="recordUsageModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title">Record Accessory Usage</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <form id="usageForm">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select name="ticket_id" class="form-select" required>
                            ...
                            @foreach(\App\Models\Ticket::where('technician_id', auth()->id())->whereIn('status', ['assigned','accepted','in_progress','scheduled'])->orderBy('ticket_no','desc')->take(50)->get() as $t)
...
$(document).ready(function() {
    $('#usageForm').on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: '{{ route("technician.inventory-management.accessories.store") }}',
    ...
    $('#returnForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ url("technician/inventory-management/accessories") }}/' + usageId + '/return',
            method: 'POST', data: new FormData(this), processData: false, contentType: false,
            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            success: function(res) { if (res.success) { showToast(res.message, 'success'); setTimeout(() => location.reload(), 1000); } },
            error: function(xhr) { showToast(xhr.responseJSON?.message || 'Error', 'error'); }

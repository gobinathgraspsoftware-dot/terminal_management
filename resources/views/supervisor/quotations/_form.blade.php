...
$(document).ready(function() {
    ...
    $(document).on('change', '.model-select', function() {
        ...
        if (modelId) {
            $.get('{{ route("supervisor.quotations.model-price", ":id") }}'.replace(':id', modelId), function(data) {
                if (data.success) {
    ...
    $(document).on('change', '.charge-select', function() {
        ...
        if (chargeId) {
            $.get('{{ route("supervisor.quotations.charge-price", ":id") }}'.replace(':id', chargeId), function(data) {
                if (data.success) {
                    row.find('.unit-price').val(data.price);
                    row.find('.description').val(data.description);
                    calculateLineTotal(row);

...
<div class="container-fluid">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('supervisor.inventory.stock-in.process') }}" method="POST" id="stockInForm">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div id="new-router-fields" style="display:none;">
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="item_name" id="item_name" class="form-control" value="{{ old('item_name') }}" maxlength="150">
                            </div>
                            <div class="col-md-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="reason" id="reason" class="form-control" value="{{ old('reason', 'Stock In') }}" maxlength="500">
                    </div>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-success"><i class="bi bi-box-arrow-in-down me-1"></i> Process Stock In</button>
                    <a href="{{ route('supervisor.inventory.index') }}" class="btn btn-outline-secondary ms-2">Cancel</a>
                </div>
            </form>
        </div>
    </div>
...
$(function() {
    ...
    $('#inventory_item_id').on('change', function() {
        ...
        if ($(this).val()) {
            $.get('{{ route("supervisor.inventory.get-item-stock") }}', { item_id: $(this).val() }, function(data) {
                $('#current-stock-info').html('<i class="bi bi-box me-1"></i>Warehouse: <strong>' + data.warehouse_stock + '</strong>');
            });
        } else {

@extends('layouts.app')
@section('title', 'Status Report — TMS')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1"><i class="bi bi-pie-chart me-2"></i>Status Report</h4>
    ...
    @include('components.reports.filter-panel', [
        'filters' => ['date_range','status','supervisor','technician','job_type','sla_breach'],
                ...
                <table id="reportTable" class="table table-sm table-hover mb-0" style="width:100%">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Job Type</th><th>Supervisor</th><th>Technician</th><th>Status</th><th>Priority</th><th>Created</th>
                        </tr>
                    </thead>
                </table>
            </div>
...
$(document).ready(function() {
    reportTable = $('#reportTable').DataTable({
        ...
        ajax: { url: '{{ route("supervisor.reports.status.data") }}', type: 'GET', data: function(d) { return $.extend(d, getReportFilters()); } },
        columns: [
            ...
            { data: 'job_type' }, { data: 'supervisor' }, { data: 'technician' }, { data: 'status' }, { data: 'priority' }, { data: 'created_at' },
        ],
        order: [], pageLength: 25, language: { emptyTable: 'No tickets found.' }
    });

    $('#btnExportExcel').on('click', function() { window.location.href = '{{ route("supervisor.reports.status.export") }}?' + $.param(getReportFilters()) + '&format=xlsx'; });
    $('#btnExportPdf').on('click', function() { window.location.href = '{{ route("supervisor.reports.status.export") }}?' + $.param(getReportFilters()) + '&format=pdf'; });
    $('#btnPrint').on('click', function() { openPrintView('status'); });
});
</script>
@endpush

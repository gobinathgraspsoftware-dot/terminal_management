...
$(document).ready(function() {
    reportTable = $('#reportTable').DataTable({
        ...
        ajax: { url: '{{ route("supervisor.reports.sla.data") }}', type: 'GET', data: function(d) { return $.extend(d, getReportFilters()); } },
        columns: [
    ...
    $('#btnExportExcel').on('click', function() { window.location.href = '{{ route("supervisor.reports.sla.export") }}?' + $.param(getReportFilters()) + '&format=xlsx'; });
    $('#btnExportPdf').on('click', function() { window.location.href = '{{ route("supervisor.reports.sla.export") }}?' + $.param(getReportFilters()) + '&format=pdf'; });
});
</script>
@endpush

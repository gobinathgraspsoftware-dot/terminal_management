...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-4">
        <div class="stats-card text-center">
            ...
            <div class="stats-label">Received This Month</div>
        </div>
    </div>
</div>
...
$(function () {
    var table = $('#paymentHistoryTable').DataTable({
        ...
        ajax: {
            url: '{{ route("supervisor.claims.payment-history-data") }}',
            data: function(d){ d.category = $('#categoryFilter').val(); d.date_from = $('#dateFrom').val(); d.date_to = $('#dateTo').val(); }
        },
        columns: [
            { data: 'claim_no' }, { data: 'category', orderable: false },
            { data: 'claimant', orderable: false }, { data: 'ticket_no', orderable: false },
            { data: 'total_amount', className: 'text-end' }, { data: 'paid_at' },
            { data: 'paid_by', orderable: false }, { data: 'status', orderable: false },
    ...
    $('#btnReset').on('click', function(){ $('#categoryFilter').val(''); $('#dateFrom,#dateTo').val(''); table.ajax.reload(); });
});
</script>
@endpush

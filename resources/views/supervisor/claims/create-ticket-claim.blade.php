                    ...
                    <form id="ticket-claim-form">
                        ...
                        <div id="ticket-details" class="d-none">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('supervisor.claims.index', ['tab' => 'ticket']) }}" class="btn btn-secondary">Cancel</a>
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <ol class="mb-0 ps-3">
                        ...
                        <li class="mb-0">Only tickets without an existing claim are shown.</li>
                    </ol>
                </div>
            </div>
        </div>
...
$(function() {
    ...
    $('#ticket-select').on('change', function() {
        ...
        $.ajax({
            url: '{{ url("supervisor/claims/ajax/ticket-details") }}/' + ticketId,
            method: 'GET',
    ...
    $('#ticket-claim-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("supervisor.claims.store-ticket-claim") }}',
            method: 'POST',
            data: $(this).serialize(),
            success: function(res) {
                if (res.success) {
                    showToast(res.message, 'success');
                    setTimeout(function() { window.location.href = '{{ route("supervisor.claims.index", ["tab" => "ticket"]) }}'; }, 1500);
                } else {

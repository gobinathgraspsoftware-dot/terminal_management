...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        @can('create_tickets')
        <a href="{{ route('supervisor.tickets.create') }}" class="btn btn-primary">
        ...
        <div id="slaAlerts" class="collapse show">
            <div class="card-body p-0" style="max-height: 200px; overflow-y: auto;">
                ...
                <div class="sla-item d-flex justify-content-between align-items-center">
                    <div>
                        <a href="{{ route('supervisor.tickets.show', $bt->id) }}" class="fw-bold text-danger text-decoration-none">
                            {{ $bt->ticket_no }}
                        </a>
...
$(document).ready(function() {
        ...
        columns: [
            {
                ...
                render: function(data, type, row) {
                    ...
                    return '<a href="{{ route("supervisor.tickets.index") }}/' + row.id + '" class="fw-bold text-decoration-none">' + data + '</a>' + sla;
                }
            },
            { data: 'vendor_name' },
            {
            ...
            { data: 'created_at' },
            {
                ...
                render: function(data, type, row) {
                    ...
                    html += '<a href="{{ route("supervisor.tickets.index") }}/' + row.id + '" class="btn btn-outline-primary" title="View"><i class="bi bi-eye"></i></a>';
                    @can('edit_tickets')
                    if (row.status !== 'completed' && row.status !== 'closed') {
                        html += '<a href="{{ route("supervisor.tickets.index") }}/' + row.id + '/edit" class="btn btn-outline-warning" title="Edit"><i class="bi bi-pencil"></i></a>';
                    }
                    @endcan
                    @can('delete_tickets')
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        confirmAction('Delete Ticket?', 'This action cannot be undone.', function() {
            $.ajax({
                url: '{{ route("supervisor.tickets.index") }}/' + id,
                method: 'DELETE',

    ...
    <div class="page-header d-flex justify-content-between align-items-center">
        ...
        <div>
            @if(!in_array($ticket->status, [Ticket::STATUS_DONE_SUCCESS, Ticket::STATUS_DONE_FAIL, Ticket::STATUS_CLOSED]))
                @can('edit_tickets')
                <a href="{{ route('supervisor.tickets.edit', $ticket->id) }}" class="btn btn-outline-primary me-1"><i class="bi bi-pencil me-1"></i>Edit</a>
                @endcan
            @endif
            <a href="{{ route('supervisor.tickets.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Back to Tickets</a>
        </div>
    </div>
        ...
        <div class="col-lg-8">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-3"><label class="form-label text-muted small">SLA</label><div class="fw-semibold">{{ $ticket->sla_hours }}h</div></div>
                            ...
                            <div class="row g-2 align-items-end">
                                <div class="col-md-5">
                                    <select name="technician_id" class="form-select" required>
                                        ...
                                        {{-- Supervisor (self) as first option --}}
                                ...
                                <div class="col-md-2"><button type="submit" class="btn btn-info w-100"><i class="bi bi-person-check me-1"></i>Assign</button></div>
                            </div>
                        </form>
                    </div>
                </div>
...
$(function() {
    ...
    $('#statusForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("supervisor.tickets.change-status", $ticket->id) }}', method: 'POST', data: fd, processData: false, contentType: false,
    ...
    $('#assignForm').on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: '{{ route("supervisor.tickets.assign", $ticket->id) }}', method: 'POST', data: $(this).serialize() + '&_token={{ csrf_token() }}',
    ...
    $('#claimForm').on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: '{{ route("supervisor.tickets.update-claim", $ticket->id) }}', method: 'POST', data: $(this).serialize() + '&_token={{ csrf_token() }}',
            success: function(res) { if (res.success) { showToast(res.message, 'success'); setTimeout(function() { location.reload(); }, 1000); } else showToast(res.message, 'error'); },
            error: function(xhr) { showToast(xhr.responseJSON?.message || 'Error.', 'error'); }
        });
    });
    ...
    $('#commentForm').on('submit', function(e) {
        ...
        if (!comment) return;
        $.ajax({
            url: '{{ route("supervisor.tickets.comment", $ticket->id) }}', method: 'POST', data: { comment: comment, _token: '{{ csrf_token() }}' },
            success: function(res) {
                if (res.success) {
                    ...
                    $('#comment_input').val(''); showToast('Comment added.', 'success');
                }
            }
        });
    });

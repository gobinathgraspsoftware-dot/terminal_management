...
class RateCardController extends Controller
{
    ...
    /**
     * Display rate cards applicable to technician
     */
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            // Only show active and effective rate cards to technicians
            $request->merge(['status' => 'active', 'effective_only' => true]);
        ...
        // Get technician's coverage states
        $user = Auth::user();
        $coverageStates = $user->coverage_states ? json_decode($user->coverage_states, true) : [];

        return view('technician.rate-cards.index', compact(
            'jobTypes',
            'calculationTypes',
            'models',
            'coverageStates'
    ...
    /**
     * Find applicable rate card for technician's job
     */

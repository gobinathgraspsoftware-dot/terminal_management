...
class StockAdjustmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $query = StockAdjustment::with(['depot', 'creator', 'approver'])
                ->where('created_by', auth()->id()) // Supervisor sees only own adjustments
    ...
    public function varianceReport(Request $request)
    {
        ...
        if ($request->ajax() && $request->has('generate')) {
            ...
            // Add filter for own adjustments only
            $variances = $this->adjustmentService->getVarianceReport($filters);
            // Filter to only supervisor's own adjustments
            $variances = $variances->filter(function($item) {
                return $item->stockAdjustment && $item->stockAdjustment->created_by === auth()->id();
            });
            return response()->json(['success' => true, 'data' => $variances->values()]);
        }

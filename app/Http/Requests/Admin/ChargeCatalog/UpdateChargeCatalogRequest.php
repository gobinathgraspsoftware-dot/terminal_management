...
/**
 * Update Charge Catalog Request
 *
 * Validation rules for updating an existing charge catalog entry
 */
class UpdateChargeCatalogRequest extends FormRequest
{
    /**
    ...
    {
        return $this->user()->can('edit_charges');
    }
    ...
    {
        $chargeId = $this->route('chargeCatalog')->id ?? null;
        ...
        return [
            'charge_code' => [
                ...
                'string',
                'max:50',
                Rule::unique('charge_catalog', 'charge_code')->ignore($chargeId),
                'regex:/^CHG\d{6}$/',
            ],
            'charge_name' => [
                'required',

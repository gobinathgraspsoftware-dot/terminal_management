...
class UpdateInventorySerialRequest extends FormRequest
{
    ...
    {
        $serialId = $this->route('inventorySerial')?->id ?? $this->route('inventory_serial');

        return [
            'serial_no'             => "required|string|max:100|unique:inventory_serials,serial_no,{$serialId}",
    ...
    {
        return [
            'serial_no.unique'             => 'This serial number already exists for another record.',
